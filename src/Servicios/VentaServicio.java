...
public class VentaServicio {
    ...
    public int RegistrarVenta(VentaModelo v) {
        String sql = "INSERT INTO tb_ventas (cliente, vendedor, total, fecha) VALUES (?,?,?,?)";
        try {
            con = cn.getConnection();
            ps = con.prepareStatement(sql);
    ...
    public int RegistrarDetalle(DetalleModelo Dv) {
        String sql = "INSERT INTO tb_detalle (id_pro, cantidad, precio, id_venta) VALUES (?,?,?,?)";
        try {
            con = cn.getConnection();
            ps = con.prepareStatement(sql);
    ...
    public boolean ActualizarStock(int cant, int id){
        String sql = "UPDATE tb_productos SET stock = ? WHERE id = ?";
        try {
            con = cn.getConnection();
            ps = con.prepareStatement(sql);
            ps.setInt(1,cant);
    ...
    public List Listarventas(){
       List<VentaModelo> ListaVenta = new ArrayList<>();
       String sql = "SELECT c.id AS id_cli, c.nombre, v.* FROM tb_clientes c INNER JOIN tb_ventas v ON c.id = v.cliente";
       try {
           con = cn.getConnection();
           ps = con.prepareStatement(sql);
           rs = ps.executeQuery();
    ...
    public VentaModelo BuscarVenta(int id){
        VentaModelo cl = new VentaModelo();
        String sql = "SELECT * FROM tb_ventas WHERE id = ?";
        try {
            con = cn.getConnection();
            ps = con.prepareStatement(sql);
            ps.setInt(1, id);
    ...
    public void pdfV(int idventa, int Cliente, double total, String usuario) {
        try {
            ...
            doc.open();
            Image img = Image.getInstance(getClass().getResource("/Img/icono_empresa.png"));
            //Fecha
            ...
            Encabezado.addCell("");
            //info empresa
            String config = "SELECT * FROM tb_config";
            String mensaje = "";
            try {
                con = cn.getConnection();
            ...
            String prove = "SELECT * FROM tb_clientes WHERE id = ?";
            try {
            ...
            tabla.addCell(c3);
            tabla.addCell(c4);
            String product = "SELECT d.id, d.id_pro,d.id_venta, d.precio, d.cantidad, p.id, p.nombre FROM tb_detalle d INNER JOIN tb_productos p ON d.id_pro = p.id WHERE d.id_venta = ?";
            try {
